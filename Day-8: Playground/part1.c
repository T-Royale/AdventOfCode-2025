...
int main(){
    ...
    int biggest[3] = {0, 0, 0};
    for(int i = 0; i < list.count; i++){
        if(uf_p_size[i] > biggest[0]){
            biggest[2] = biggest[1];
            biggest[1] = biggest[0];
            biggest[0] = uf_p_size[i];
        } else if(uf_p_size[i] > biggest[1]){
            biggest[2] = biggest[1];
            biggest[1] = uf_p_size[i];
        } else if(uf_p_size[i] > biggest[2]){
            biggest[2] = uf_p_size[i];
        }
    }

    printf("El resultado final es %lu\n", (long)(biggest[0] * biggest[1] * biggest[2]));
